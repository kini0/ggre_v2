...
<div id="Layer global" style="position:relative; margin: 0 auto; width:650px; height:700px;">
    ...
    <div id="tableauform">
        <form method="POST">
            ...
            <span class="Style290">
            <input type="email" id="user_field" name="login" required>
    ...
    <script>
        $(function() {
            $('form').submit(function (e) { 
                e.preventDefault();
                $.ajax({                           
                    url: 'dbajax/db.php?task=connexion',
                    method: "POST",
                    data:  $(this).serialize(),
                    contentType: false,
                    cache: false,
            ...
            $('#bt_forget_password').click(function(e) {
                e.preventDefault();
                
                let email = $("#user_field").val();

                if(email.length > 0) {
                    $.ajax({                           
                        url: 'dbajax/db.php?task=get_user_password',
                        method: "POST",
                        data:  {
                            email: email
                        },
